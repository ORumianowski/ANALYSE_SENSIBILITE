...
## 1.D. Fournissez le schéma des transitions entre états concordant avec ces équations
  ...
  ![Texte alternatif](image/schema_projet_mepi.PNG){width=50%}


  
```{r}
# Charger le package diagrammeR
library(DiagrammeR)

# Créer un graphique SEIR
graph <- create_graph() %>%
  
  # Nœud pour la première classe d'âge
  add_node("S1", label = "S1", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "lightblue")) %>%
  add_node("S2", label = "S2", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "lightblue")) %>%
  add_node("S3", label = "S3", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "lightblue")) %>%
  
  add_node("E1", label = "E1", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "orange")) %>%
  add_node("E2", label = "E2", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "orange")) %>%
  add_node("E3", label = "E3", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "orange")) %>%
  
  add_node("I1", label = "I1", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "red")) %>%
  add_node("I2", label = "I2", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "red")) %>%
  add_node("I3", label = "I3", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "red")) %>%
  
  add_node("R1", label = "R1", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "green")) %>%
  add_node("R2", label = "R2", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "green")) %>%
  add_node("R3", label = "R3", node_aes = node_aes(shape = "rectangle", style = "filled", fillcolor = "green")) %>%
  
  # Arêtes pour les transitions
  # transition entre classe d'âge 1 et 2
  add_edge(from="S1", to="S2", edge_aes = edge_aes(label = "t1", color = "black")) %>%
  add_edge(from="E1", to="E2", edge_aes = edge_aes(label = "t1", color = "black")) %>%
  add_edge(from="I1", to="I2", edge_aes = edge_aes(label = "t1", color = "black")) %>%
  add_edge(from="R1", to="R2", edge_aes = edge_aes(label = "t1", color = "black")) %>%
  
  # transition entre classe d'âge 2 et 3
  add_edge(from="S2", to="S3", edge_aes = edge_aes(label = "t2", color = "black")) %>%
  add_edge(from="E2", to="E3", edge_aes = edge_aes(label = "t2", color = "black")) %>%
  add_edge(from="I2", to="I3", edge_aes = edge_aes(label = "t2", color = "black")) %>%
  add_edge(from="R2", to="R3", edge_aes = edge_aes(label = "t2", color = "black")) %>%

  # transition entre état de santé
  add_edge(from="S1", to="E1", edge_aes = edge_aes(label = "trans", color = "black")) %>%
  add_edge(from="E1", to="I1", edge_aes = edge_aes(label = "lat", color = "black")) %>%
  add_edge(from="I1", to="R1", edge_aes = edge_aes(label = "rec", color = "black")) %>%
  add_edge(from="R1", to="S1", edge_aes = edge_aes(label = "loss", color = "black")) %>%
  
  add_edge(from="S2", to="E2", edge_aes = edge_aes(label = "trans", color = "black")) %>%
  add_edge(from="E2", to="I2", edge_aes = edge_aes(label = "lat", color = "black")) %>%
  add_edge(from="I2", to="R2", edge_aes = edge_aes(label = "rec", color = "black")) %>%
  add_edge(from="R2", to="S2", edge_aes = edge_aes(label = "loss", color = "black")) %>%
  
  add_edge(from="S3", to="E3", edge_aes = edge_aes(label = "trans", color = "black")) %>%
  add_edge(from="E3", to="I3", edge_aes = edge_aes(label = "lat", color = "black")) %>%
  add_edge(from="I3", to="R3", edge_aes = edge_aes(label = "rec", color = "black")) %>%
  add_edge(from="R3", to="S3", edge_aes = edge_aes(label = "loss", color = "black"))
 
  # Rajouter le titre
  #add_global(
  #  graph_title = "Modèle SEIR avec Classes d'Âge",
 #   graph_title_location = "top" )
...
for (i in 1:4){
  hist(result_fast100[,i],main=title[i],xlab="Valeurs",ylab="Fréquence",breaks=100)
}
...
La distribution de la sortie 3 étant intermédiaire entre ces 2 formes. Pour faire l'analyse de variance, l'hypothèse de normalité de la distribution s'applique. Les sorties 2 et 4 ne pourront donc pas être utilisé dans ce cadre pour calculer les indices de sensibilité. La sortie 1 et 3 se rapproche assez d'une distribution normale pour que l'on puisse les garder.
...
result_fast100 = result_fast100[,c(1,3)]
result_fast1000 = result_fast1000[,c(1,3)]

# calcule des indices de sensibilité avec tell()
indice_sensibilite_100 = tell(Fast100,result_fast100)
indice_sensibilite_1000 = tell(Fast100,result_fast100)

indice_sensibilite_100
```
